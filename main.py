...
ucant = nextcord.Embed(title="📣WARNING📣",description="❌ คุณไม่สามารถใช้คำสั่งในห้องนี้ได้กรุณาไปใช้ในห้อง #📲•ʙᴏᴛꜱᴘᴀᴍꜱᴍꜱ ❌",color=0xFF0000)
...
async def on_ready():
    ...
    await bot.change_presence(activity=nextcord.Streaming(name="ʙᴏᴛꜱᴘᴀᴍꜱᴍꜱꜰʀᴇᴇʙʏ:ʙꜰʜᴍꜱᴄ",url="https://www.twitch.tv/bfhmsc"))
...
class Sms(nextcord.ui.Modal):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        if self.amount.value > config['limit']:
            embed=nextcord.Embed( description=f"📱 ใส่ไม่เกิน {config['limit']} นาที.", color=0x00ff00)
            embed.set_footer(text="BOTBY:BFHMSC")
            embed.set_author(name='BOTSPAMOTPFREE')
            ...
            if phone in config['blacklist']:
                embed=nextcord.Embed( description="🔐 ไม่สามารถยิงเบอร์นี้ได้ 🔐", color=0x00ff00)
                embed.set_footer(text="BOTBY:BFHMSC")
                embed.set_author(name='BOTSPAMOTPFREE')
                embed.set_image(url=config['imgurl'])
                embed.timestamp = datetime.datetime.utcnow()
                ...
                try:
                    done=nextcord.Embed(title="**บอทยิงเบอร์ฟรี**", description=f"- คุณ <@{interaction.user.id}> กำลัง  \n- ยิงไปที่เบอร์  {phone} \n- เวลาที่ยิงไปทั้งหมด {amount_int} นาที \n API : 108 API \n STATUS : RANDOM \n โค้ดบอทยิงเบอร์ 50 บาทสนใจก็ซื้อได้ ", color=0x00ff00 )
                    done.set_footer(text="BOTBY:BFHMSC")
                    done.set_image(url=config['imgurl'])
                    done.timestamp = datetime.datetime.utcnow()
                    await interaction.send(embed=done, ephemeral=True)
                    log = nextcord.Embed(title="**แจ้งเตือนการยิงเบอร์**", description=f"📌 คุณ <@{interaction.user.id}> กำลังยิงไปที่เบอร์  `{phone}` \n🕜 ยิงไป {amount_int} นาที", color=0x00ff00 )
...
@bot.slash_command(guild_ids=[guildid],description="🟢 บอทยิงเบอร์ 🟢")
async def sms(interaction: nextcord.Interaction):
    if interaction.channel_id in config['channel_id']:
        await interaction.response.send_modal(Sms())
    else:
        await interaction.send(embed=ucant,ephemeral=True)
